...
function ImportExportPage() {
  ...
  const handleSyncToCloud = useCallback(async () => {
    ...
    try {
      ...
      if (enqueued === 0) {
        toast(t('importPage.noItemsToSync' as never), { icon: 'ℹ️' })
        ...
        setSyncStarted(true)
        setShowSyncPrompt(false)
        toast.success(t('importPage.itemsAwaitingSync' as never, { count: enqueued }), {
          duration: 3000,
        })
      ...
      toast.error(t('importPage.enqueueFailed' as never))
    } finally {
      setIsSyncing(false)
    }
  }, [navigate, t])
                ...
                {syncStarted && pendingCount > 0 && (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h4 className="font-semibold text-blue-900 dark:text-blue-100">
                            {t('importPage.syncInProgress' as never)}
                          </h4>
                          <p className="text-blue-700 text-sm dark:text-blue-200">
                            {t('importPage.itemsRemaining' as never, { count: pendingCount })}
                          </p>
                        </div>
                      </div>
                      <button
                      ...
                      >
                        {t('importPage.goToReceipts' as never)}
                      </button>
                    </div>
                  </motion.div>
                )}
                ...
                {syncStarted && pendingCount === 0 && (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h4 className="font-semibold text-green-900 dark:text-green-100">
                            {t('importPage.syncComplete' as never)}
                          </h4>
                          <p className="text-green-700 text-sm dark:text-green-200">
                            {t('importPage.syncCompleteDescription' as never)}
                          </p>
                        </div>
                      </div>
                      <button
                      ...
                      >
                        {t('importPage.viewReceipts' as never)}
                      </button>
                    </div>
                  </motion.div>
                )}
