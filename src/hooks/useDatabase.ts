...
import { db, type Device } from '@lib/db'
...
export function useReceipts() {
  return useLiveQuery(
    () => db.receipts.orderBy('date').reverse().toArray(),
    []
  )
}
...
export function useReceipt(id: number | undefined) {
  return useLiveQuery(
    () => (id !== undefined ? db.receipts.get(id) : undefined),
    [id]
  )
}
...
export function useReceiptsByCategory(category: string) {
  return useLiveQuery(
    () => dbAPI.getReceiptsByCategory(category),
    [category]
  )
}
...
export function useReceiptSearch(query: string) {
  return useLiveQuery(
    () => query ? dbAPI.searchReceipts(query) : db.receipts.toArray(),
    [query]
  )
}
...
export function useDevice(id: number | undefined) {
  return useLiveQuery(
    () => (id !== undefined ? db.devices.get(id) : undefined),
    [id]
  )
}
...
export function useDevicesByStatus(status: Device['status']) {
  return useLiveQuery(
    () => db.devices.where('status').equals(status).toArray(),
    [status]
  )
}
...
export function useExpiringDevices(days = 30) {
  return useLiveQuery(
    () => dbAPI.getDevicesByWarrantyStatus(days),
    [days]
  )
}
...
export function useDeviceSearch(query: string) {
  return useLiveQuery(
    () => query ? dbAPI.searchDevices(query) : db.devices.toArray(),
    [query]
  )
}
...
export function useDeviceReminders(deviceId: number | undefined) {
  return useLiveQuery(
    () => deviceId !== undefined
      ? db.reminders.where('deviceId').equals(deviceId).toArray()
      : [],
    [deviceId]
  )
}
...
export function usePendingReminders() {
  return useLiveQuery(
    () => db.reminders.where('status').equals('pending').toArray(),
    []
  )
}
